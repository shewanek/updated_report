...
def main():
    ...
    if mydb is not None:
        cursor = mydb.cursor()
        df_combine = load_districtconversiondata(mydb)
        ...
        col1.metric(label="**Total Register Customer**", value=df_combine.convId.nunique(), delta="Customer")
        ...
        branch_df = df_combine.groupby(by=["Branch"], as_index=False)["convId"].count()
        branch_df.rename(columns={"convId": "Total Registered"}, inplace=True)
        ...
        with col5:
            ...
            product_df = df_combine.groupby(by=["Product Type"], as_index=False)["convId"].count()
            product_df.rename(columns={"convId": "Total Registered"}, inplace=True)
            ...
            fig_product.update_layout(
                ...
                annotations=[
                    dict(
                        ...
                        x=0.5,  # Centering the text horizontally
                        y=1.1,  # Positioning the text above the chart
                        showarrow=True,
                        font=dict(size=14)
                    )
                ]
        ...
        with col4:
            ...
            grouped_df = df_combine.groupby(["District", "Branch"]).agg(Total_Registered=("convId", "count")).reset_index().rename(lambda x: x + 1)
