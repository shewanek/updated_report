...
def main():
    ...
    html_title = """
        ...
        <center> <h3 class="title_dash"> Michu Rejected Data Upload Portal </h3> </center>
        """
    with col2:
    ...
    uploaded_file = st.file_uploader(":orange[Upload Rejected Excel file 👇🏻]", type=["csv", "txt", "xlsx", "xls"], accept_multiple_files=False)
    # make_sidebar1()

    try:
        if uploaded_file is not None:
            from dependence import all_branch_code_exist, upload_rejected
            ...
            try:
                column_types = {
                    ...
                    'CustomerAddress__region': str,
                    'CustomerAddress__subcity_zone': str,
                    'CustomerAddress__woreda': str,
                    'statuss' : str,
                ...
                required_columns = {'branch_code', 'michu_id', 'customer_name', 'phone_number', 'customer_number', 'saving_account', 'business_tin_number', 'gender', 'michu_loan_product', 'prefered_language', 'customeraddress__region', 'customeraddress__subcity_zone', 'customeraddress__woreda', 'statuss', 'rejection_reason_remark', 'customer_feedback'}
                # st.write(df.columns)
                missing_columns = required_columns - set(df.columns)
                req_col = {'branch_code', 'michu_id'}
                if missing_columns:
