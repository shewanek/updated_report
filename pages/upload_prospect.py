...
def main():
    ...
    html_title = """
        ...
        <center> <h3 class="title_dash"> Michu Prospect Data Upload Portal </h3> </center>
        """
    with col2:
    ...
    uploaded_file = st.file_uploader(":orange[Upload prospect Excel file 👇🏻]", type=["csv", "txt", "xlsx", "xls"], accept_multiple_files=False)
    # make_sidebar1()

    try:
        if uploaded_file is not None:
            from dependence import all_branch_code_exist, upload_prospect
            filename = uploaded_file.name
            # Determine file extension
            file_extension = Path(filename).suffix.lower()
            try:
                column_types = {
                    'customer_id_michu': str,
                    'age': str, 
                    'branch_code': str, 
                    'phone': str,
                    'full_name': str,
                    'gender': str,
                    'customer_account_current_stage': str,
                ...
                required_columns = {'customer_id_michu', 'age', 'branch_code', 'phone', 'full_name', 'gender', 'customer_account_current_stage', 'michu_loan_product', 'saving_account', 'customer_number', 'marital_status', 'region', 'statuss'}
                # st.write(df.columns)
                ...
                if missing_columns:
                    ...
                    if required_df.isnull().any().any():
                        ...
                        df['customer_id_michu'] = df['customer_id_michu'].str.strip()
