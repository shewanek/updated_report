...
def any_target_date_exists(df, mydb):
    ...
    if len(formatted_dates) == 1:
        query = "SELECT target_date FROM target WHERE target_date = %s"
        cursor.execute(query, (formatted_dates[0],))
    else:
        query = "SELECT target_date FROM target WHERE target_date IN %s"
...
def upload_to_db(df, mydb):
    ...
    for index, row in df.iterrows():
        formatted_date = format_target_date(row['target_date'])
        cursor.execute("""
            INSERT INTO target (branch_code, unique_target, account_target, disbursment_target, target_date)
            VALUES (%s, %s, %s, %s, %s)
            """,
            (row['branch_code'], 
             row['unique_target'], 
             row['account_target'], 
             row['disbursment_target'], 
             formatted_date)
        )
    mydb.commit()
    cursor.close()
...
def main():
    ...
    html_title = """
        ...
        <center> <h3 class="title_dash"> Michu Target Data Upload Portal </h3> </center>
        """
    with col2:
    ...
    uploaded_file = st.file_uploader(":orange[Upload Target Excel file 👇🏻]", type=["csv", "txt", "xlsx", "xls"], accept_multiple_files=False)
    make_sidebar1()
    ...
    if mydb is not None:
        if uploaded_file is not None:
            ...
            try:
                ...
                required_columns = {'branch_code', 'unique_target', 'account_target', 'disbursment_target', 'target_date'}
                ...
                if missing_columns:
                    ...
                    target_columns = ['unique_target', 'account_target', 'disbursment_target']
                    
                    for col in target_columns:
                        df[col] = df[col].fillna(0)
                        # df[col] = pd.to_numeric(df[col], errors='coerce')
                        # df[col] = np.floor(df[col] + 0.5).astype(int)
                    ...
                    try:
                        ...
                        invalid_dates = df['target_date'].isna()
                        if invalid_dates.any():
                            st.error("The target_date column contains invalid dates.")
                            return
                    ...
                    if missing_branch_code:
                        ...
                        if any_target_date_exists(df, mydb):
                            st.error("One or more target dates in the uploaded file already exist in the database. Upload aborted.")
                        else:
...
# streamlit run main.py --server.port 4060 --server.address 0.0.0.0