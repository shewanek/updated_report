...
def kiyya_register_withid(n_name, phone_nmuber, Saving_Account, national_id,  gender, date_of_birth, transactionID, national_phone, email, address, responseTime):
    ...
    with st.form(key='customer_form', clear_on_submit= True):
        ...
        name = st.text_input('Full Name', key=name_key, value=n_name, disabled=True).strip()
        ...
        with col1:
            if st.form_submit_button(':blue[Register]'):
                from dependence import kiyya_customerwith_nid, get_natinal_id # kiyya_customer_notegible
                ...
                # Check if the user has entered a value for Daily Sales
                    ...
                    if kiyya_customerwith_nid(username, name, phone_nmuber, Saving_Account, customer_id_type, gender, marital_status, date_of_birth, region, zone_subcity, woreda, educational_level, economic_sector, line_of_business, initial_working_capital, source_of_initial_capital, monthly_income, purpose_of_loan, national_id, transactionID, national_phone, email, address, responseTime):
