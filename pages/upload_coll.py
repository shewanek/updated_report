...
def main():
    ...
    try:
        if uploaded_file is not None:
            from dependence import all_branch_code_exist, upload_coll, any_actula_coll_exists
            ...
            try:
                ...
                expected_columns = [
                    "branch", "customer number", "loan id", "customer name", "phone number",
                    "business tin number", "loan product name", "product group", "application status",
                    "approved date", "maturity date", "approved amount", "collection date",
                    "principal collected", "interest collected", "penalty collected", "total collected",
                    "collected from", "reason"
                ]
                ...
                column_mapping = {
                    ...
                    "loan product name": "michu_loan_product",
                ...
                if not all(col in df.columns for col in expected_columns):
                    ...
                    date_cols = ['approved_date', 'maturity_date', 'collection_date']
                    ...
                    for col in ['approved_amount', 'principal_collected', 'interest_collected', 'penalty_collected', 'total_collected']:
                        df[col] = pd.to_numeric(df[col], errors='coerce').round(2)

                    # Convert all other columns to string (if not already)
                    for col in df.columns:
                        if col not in ['approved_date', 'maturity_date', 'collection_date',
                    ...
                    df = df.groupby('loan_id', as_index=False).agg({
                        ...
                        'penalty_collected': 'sum',
                        'total_collected': 'sum',
                        'collected_from': 'first',
                        'reason': 'first'
                    })
