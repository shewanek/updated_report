...
def main():
    ...
    try:
        ...
        if active_tab == "Wabbi & Kiyya-Formal":
            ...
            df_combine_collection, df_combine_conversion, df_combine_arrears = load_arrers_conver_detail(role, username)
            # st.write(df_combine_arrears)
            # df_combine_arrears['michu_loan_product'] = df_combine_arrears['michu_loan_product'].replace({'Michu 1.0', 'Michu Wabii'})
            ...
            if df_combine_conversion is not None and not df_combine_conversion.empty:
                start_dates.append(df_combine_conversion["approved_date"].min())
                end_dates.append(df_combine_conversion["approved_date"].max())
            ...
            df_combine_conversion = df_combine_conversion[(df_combine_conversion["approved_date"] >= date1) & (df_combine_conversion["approved_date"] <= date2)]
            ...
            total_in_arrears = df_combine_arrears['loan_id'].nunique()
            total_collected =  df_combine_collection['Loan_Id'].nunique()
            total_conversion = df_combine_conversion['loan_id'].nunique()
            ...
            amount_target = float(df_combine_arrears['outstanding_balance'].sum())
            ...
            # Usage in Streamlit
            if role == "Admin" or role == 'under_admin' or role == 'collection_admin' or role == 'Sales Admin':
                ...
                with coll1:
                    ...
                    total_customers_in_arrears = (df_combine_arrears.groupby(['District']).agg({'loan_id': 'count'}).reset_index())
                    total_customers_in_arrears.rename(columns={'loan_id': 'Total In Arrears'}, inplace=True)

                    # Calculate Total Conversion
                    total_conversion = (df_combine_conversion.groupby(['District']).agg({'loan_id': 'count'}).reset_index())
                    total_conversion.rename(columns={'loan_id': 'Total Conversion'}, inplace=True)
                ...
                with coll2:
                    ...
                    total_customers_in_arrears = (df_combine_arrears.groupby(['District', 'Branch']).agg({'loan_id': 'count'}).reset_index())
                    total_customers_in_arrears.rename(columns={'loan_id': 'Total In Arrears'}, inplace=True)

                    # Calculate Total Conversion
                    total_conversion = (df_combine_conversion.groupby(['District', 'Branch']).agg({'loan_id': 'count'}).reset_index())
                    total_conversion.rename(columns={'loan_id': 'Total Conversion'}, inplace=True)
            ...
            st.markdown("""
                ...
                """, unsafe_allow_html=True)
            tab2, tab3, tab4 = st.tabs(["Michu Wabbi", "Michu Kiyya-Formal", "Michu 1.0"])
            

            with tab2:
                tab21, tab22, tab23 = st.tabs(["In Arrears", "Conversion(Active", "Collected"])
                if role == 'District User':
                        st.markdown("""
                        <style>
                ...
                with tab21:
                    ...
                    st.write(
                        df_combine_arrears[df_combine_arrears["michu_loan_product"] == "Michu Wabii"]
                ...
                with tab22:
                    ...
                    st.write(df_combine_conversion[df_combine_conversion["michu_loan_product"] == "Michu Wabii"]
                             .drop(columns=['loan_id', 'outstanding_balance', 'approved_amount', 'approved_date'])
                             .reset_index(drop=True)
                             .rename_axis("Index")
                             .rename(lambda x: x + 1))
                with tab23:
                    ...
                    st.write(df_combine_collection[df_combine_collection["Michu Loan Product"] == "Wabbi"]
                             .drop(columns=['Loan_Id'])
                             .reset_index(drop=True)
                             .rename_axis("Index")
                             .rename(lambda x: x + 1))
            with tab3:
                tab31, tab32, tab33 = st.tabs(["In Arrears", "Active", "Collected"])
                with tab31:
                    st.markdown('<span style="color: #e38524;">**Michu Customer** (<span style="color: #00adef;">In Arrears Status </span>)</span> 👇🏻', unsafe_allow_html=True)
                    st.write(
                        df_combine_arrears[df_combine_arrears["michu_loan_product"] == "Michu Kiyya Formal"]
                        .drop(columns=["loan_id"])
                        .reset_index(drop=True)
                        .rename_axis("Index")
                        .rename(lambda x: x + 1)
                ...
                with tab32:
                    st.markdown('<span style="color: #e38524;">**Michu Customer** (<span style="color: #00adef;">Active Status </span>)</span> 👇🏻', unsafe_allow_html=True)
                    st.write(df_combine_conversion[df_combine_conversion["michu_loan_product"] == "Michu Kiyya Formal"]
                             .drop(columns=['loan_id', 'outstanding_balance', 'approved_amount', 'approved_date'])
                             .reset_index(drop=True)
                             .rename_axis("Index")
                             .rename(lambda x: x + 1))
                with tab33:
                    st.markdown('<span style="color: #e38524;">**Michu Customer** (<span style="color: #00adef;">Closed Status </span>)</span> 👇🏻', unsafe_allow_html=True)
                    st.write(df_combine_collection[df_combine_collection["Michu Loan Product"] == "Women Formal"]
                             .drop(columns=['Loan_Id'])
                             .reset_index(drop=True)
                             .rename_axis("Index")
                             .rename(lambda x: x + 1))
            ...
            with tab4:
                tab41, tab42, tab43 = st.tabs(["In Arrears", "Active", "Collected"])
                with tab41:
                    st.markdown('<span style="color: #e38524;">**Michu Customer** (<span style="color: #00adef;">In Arrears Status </span>)</span> 👇🏻', unsafe_allow_html=True)
                    st.write(
                        df_combine_arrears[df_combine_arrears["michu_loan_product"] == "Michu 1.0"]
                        .drop(columns=["loan_id"])
                        .reset_index(drop=True)
                        .rename_axis("Index")
                        .rename(lambda x: x + 1)
                ...
                with tab42:
                    st.markdown('<span style="color: #e38524;">**Michu Customer** (<span style="color: #00adef;">Active Status </span>)</span> 👇🏻', unsafe_allow_html=True)
                    st.write(df_combine_conversion[df_combine_conversion["michu_loan_product"] == "Michu 1.0"]
                             .drop(columns=['loan_id', 'outstanding_balance', 'approved_amount', 'approved_date'])
                             .reset_index(drop=True)
                             .rename_axis("Index")
                             .rename(lambda x: x + 1))
                with tab43:
                    st.markdown('<span style="color: #e38524;">**Michu Customer** (<span style="color: #00adef;">Closed Status </span>)</span> 👇🏻', unsafe_allow_html=True)
                    st.write(df_combine_collection[df_combine_collection["Michu Loan Product"] == "Michu 1.0"]
                             .drop(columns=['Loan_Id'])
                             .reset_index(drop=True)
                             .rename_axis("Index")
                             .rename(lambda x: x + 1))
            ...
            if role == 'District User':
                ...
                # 🔽 Add download button
                download_df = grouped_df.copy()  # raw numbers without formatting
                csv = download_df.to_csv(index=False).encode('utf-8')
                st.download_button(
                    label="📥 Download Summary CSV",
                    data=csv,
                    file_name="district_branch_summary.csv",
                    mime="text/csv"
                )
            ...
            # unique_customer, unique_by_self, registed_by_branch, df_unique_all = load_unquiecustomer(role, username)
            df_combine_collection, df_combine_conversion, df_combine_arrears = load_arrers_informal_detail(role, username)
            # Combine unique values for filters
            combined_districts = sorted(set(df_combine_collection["District"].dropna().unique()) | set(df_combine_conversion["District"].dropna().unique()) | set(df_combine_arrears["District"].dropna().unique()))
            combined_branches = sorted(set(df_combine_collection["Branch"].dropna().unique()) | set(df_combine_conversion["Branch"].dropna().unique()) | set(df_combine_arrears["Branch"].dropna().unique()))
            ...
            st.markdown("""
                ...
                """, unsafe_allow_html=True)
            tab2, tab3 = st.tabs(["Michu Guyya", "Michu Kiyya-InFormal"])
            ...
            with tab2:
                ...
                with tab21:
                    st.markdown('<span style="color: #e38524;">**Michu Customer** (<span style="color: #00adef;">In Arrears Status </span>)</span> 👇🏻', unsafe_allow_html=True)
                    st.write(
                        df_combine_arrears[df_combine_arrears["michu_loan_product"] == "Michu Guyya"]
                        .drop(columns=["loan_id"])
                        .reset_index(drop=True)
                        .rename_axis("Index")
                        .rename(lambda x: x + 1)
                ...
                with tab22:
                    st.markdown('<span style="color: #e38524;">**Michu Customer** (<span style="color: #00adef;">Active Status </span>)</span> 👇🏻', unsafe_allow_html=True)
                    st.write(df_combine_conversion[df_combine_conversion["michu_loan_product"] == "Michu Guyya"]
                             .drop(columns=['loan_id', 'outstanding_balance', 'approved_amount', 'approved_date'])
                             .reset_index(drop=True)
                             .rename_axis("Index")
                             .rename(lambda x: x + 1))
                with tab23:
                    st.markdown('<span style="color: #e38524;">**Michu Customer** (<span style="color: #00adef;">Closed Status </span>)</span> 👇🏻', unsafe_allow_html=True)
                    st.write(df_combine_collection[df_combine_collection["Michu Loan Product"] == "Guyya"]
                             .drop(columns=['Loan_Id'])
                             .reset_index(drop=True)
                             .rename_axis("Index")
                             .rename(lambda x: x + 1))
            with tab3:
                ...
                with tab31:
                    st.markdown('<span style="color: #e38524;">**Michu Customer** (<span style="color: #00adef;">In Arrears Status </span>)</span> 👇🏻', unsafe_allow_html=True)
                    st.write(
                        df_combine_arrears[df_combine_arrears["michu_loan_product"] == "Michu Kiyya Informal"]
                        .drop(columns=["loan_id"])
                        .reset_index(drop=True)
                        .rename_axis("Index")
                        .rename(lambda x: x + 1)
                ...
                with tab32:
                    st.markdown('<span style="color: #e38524;">**Michu Customer** (<span style="color: #00adef;">Active Status </span>)</span> 👇🏻', unsafe_allow_html=True)
                    st.write(df_combine_conversion[df_combine_conversion["michu_loan_product"] == "Michu Kiyya Informal"]
                             .drop(columns=['loan_id', 'outstanding_balance', 'approved_amount', 'approved_date'])
                             .reset_index(drop=True)
                             .rename_axis("Index")
                             .rename(lambda x: x + 1))
                with tab33:
                    st.markdown('<span style="color: #e38524;">**Michu Customer** (<span style="color: #00adef;">Closed Status </span>)</span> 👇🏻', unsafe_allow_html=True)
                    st.write(df_combine_collection[df_combine_collection["Michu Loan Product"] == "Women InFormal"]
                             .drop(columns=['Loan_Id'])
                             .reset_index(drop=True)
                             .rename_axis("Index")
                             .rename(lambda x: x + 1))
