...
from dependence import connect_to_database, load_convertion
...
def main():
    ...
    st_autorefresh(interval=refresh_interval * 1000, key="Michu dash")
    ...
    if mydb is not None:
        cursor = mydb.cursor()
        df_combine = load_convertion(mydb)
        # today = datetime.today().date()
        # df_combine = df_combine[df_combine["Collection Date"] == today]
        # Side bar
        ...
        if df_combine is not None and not df_combine.empty:
            ...
            # df_combine["Collection_date"] = df_combine["Collection_date"]
        ...
        col1.metric(label="**Total Rigister Customer**", value=df_combine.ConversionId.nunique(), delta="Customer")
        ...
        category_df = df_combine.groupby(by=["District"], as_index=False)["ConversionId"].count()
        category_df.rename(columns={"ConversionId": "Total Registered"}, inplace=True)
        ...
        # # st.write(df_combine.drop(columns=['ConversionId', 'userName']))
        # st.write(df_combine.drop(columns=['ConversionId', 'userName']).reset_index(drop=True).rename(lambda x: x + 1))

        
        # df = df_combine.drop(columns=['ConversionId', 'userName'])
